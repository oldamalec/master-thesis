...
\newglossaryentry{gHeuristickaAnalyza}{
	name=Heuristická analýza,
	description={
		Metoda testování sotfwaru, při které využívá expert své znalosti, aby odhadl možné chování ostatních uživatelů. Typicky prochází několik předem stanovených bodů a zkoumá, zda jsou splněny.
	}
}
\newglossaryentry{gHotSwap}{
	name=Hot Swapping,
	description={
		Jedná se o způsob zobrazení změn v aplikaci při změně jejího kódu. Při hot-swappingu není potřeba spouštět žádné procesy sestavení aplikace a často ani načíst znovu obrazovku - vše je provedeno automaticky za běhu a změny jsou viditelné ihned.
	}
}
\newglossaryentry{gIdempotence}{
	name=Idempotence,
	description={
		Idempotence je vlastnost operací. Operace je idempotentní, pokud jejím opakovaným použitím na nějaký vstup vznikne stejný výstup, jako vznikne jediným použitím dané operace. V kontextu programu programu se tedy jedná o akci, kterou lze vyvolávat opakovaně, aniž by to způsobilo vedlejší efekty.
	}
}
\newglossaryentry{gJSdoc}{
	name=JSdoc,
	description={
		Specifikace zápisu dokumentace javascriptového kódu.
	}
}
...
\newglossaryentry{gRedmine}{
	name=Redmine,
	description={
		Software pro evidenci požadavků a řízení projektů.
	}
}
\newglossaryentry{gReflexe}{
	name=Reflexe,
	description={
		Popsání programovacího jazyka sebou samým - například možnost zjistit si uvnitř funkce, jaké jsou její parametry.
	}
}
...
\newglossaryentry{gToastMessage}{
	name=Toast message,
	description={
		Krátká informativní zpráva, která se objevuje ve spodní části aplikace a obsahuje typicky informaci o potvrzení provedení akce.
	}
}
...
\newglossaryentry{gUseCase}{
	name=Use case,
	description={
		Případ použití, neboli seznam kroků či událostí, ke kterým dochází mezi uživatelem aplikace a aplikací samotnou, jejichž údělem je dosažení nějakého cíle.
	}
}
...
\newglossaryentry{gVuex}{
	name=Vuex,
	description={
		Knihovna pro Vue.js, která realizuje state management pattern.
	}
}
...
\newglossaryentry{gWebStorm}{
	name=WebStorm,
	description={
		IDE od společnosti JetBrains pro vývoj webových aplikací především v Javascriptu či Typescriptu.
	}
}
